...
- 単独で使うコード決済・電子マネー（d払い残高・楽天ペイ・au PAY・WAON・nanaco・楽天Edy）を「カード以外の支払い」に加える。多くは0.5%前後でランキングへの影響は小さく、マスタの追加だけで推奨エンジンは変えない。新規利用者の既定はオフ。現金は常に0%なので比べず、現金しか使えない店に注記する。チャージ元カードとの組み合わせは今回は対象外（詳細設計32.11）
