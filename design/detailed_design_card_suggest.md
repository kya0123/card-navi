...
| 項目 | 内容 |
|---|---|
| 版数 | 1.2（見直すタブで年会費の回収を計算） |
| 作成日 | 2026-09-24（1.2：2026-09-25） |
...
1. **月のカード利用額**：「月のカード利用額 [ 50,000 ] 円」と −／＋ ボタン（1万円単位）。変更すると即座に再計算し、設定に保存する。説明「よく行くお店に均等に使う前提の目安です」
...
最近のお店が5件未満のときは、1〜4の代わりに既存の案内を出す（利用額の入力欄は表示する）。

### 10.5 ヒント・バナーへの影響

- ①ヒント（`bestUnownedAt`）：変更なし
