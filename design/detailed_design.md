...
| 分冊 | カード提案機能：`design/detailed_design_card_suggest.md`（v1.2。見直すタブの年会費回収計算は10章） |
...
| R2 | マニア界隈でお得とされるカードも一覧に追加する | 同じく `enthusiast`（マニア推奨）を付ける。両方に該当するカードは両方を付ける |
...
■ カード一覧表示
┌ カード一覧から選ぶ ──────────────────┐
│ [すべて 19][定番 16][マニア推奨 8][保有中 N]   ← 絞り込み │
│ [カード名・発行会社で検索            ]      │
│ ☑ 三井住友カード（NL）   [定番][マニア]         │
...
- 区分バッジ：定番＝グレー、マニア推奨＝キーカラー（DADSのタグ相当）。コントラスト4.5:1以上
...
| `--accent-line` | gold-600 `#b08d2f` | 1位の枠、「マニア推奨」タグの枠 | 白地 3.1:1（非テキストの基準3:1以上） |
