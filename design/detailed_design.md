...
| 1.19 | 2026-09-26 | UIの見直し（31章。設計のみ・未実装）：近くのお店のタグ廃止、ボーナスタブ廃止とカード診断タブへの統合（タブ名は画面案で確認）、おすすめを1カード1枠に、支払い方法の選択廃止、用語統一、ボーナスの期待ポイント率を年会費差し引き（B案）に |
...
| U2 | タブ構成 | 「見直す」タブを改名する（**「カード診断」または「ナビ」。画面案で確認**）。**ボーナスタブは廃止**してタブを5つにする（おすすめ／カード／カード診断（仮）／ポイント率／設定） |
...
|---|---|
| おすすめ | 変更は U4〜U7・U14 |
| カード | 持っているカード：カードごとに優先順（↑↓）、ボーナスのあるカードだけ「入会年月」「このボーナスを狙う」「今期は達成済み」、外すボタン。支払い方法のチェックはなし（U8）。カード一覧から選ぶ：変更なし |
| カード診断（仮） | 上部で［持っているカード｜おすすめカード］を切り替え → 月のカード利用額（1行＋［変更］、共通の値）→ カードごとの説明 |
| ポイント率 | 旧「還元情報」。カードごとに折りたたみ（U10） |
| 設定 | バックアップ案内（U7）、詳細設定（U11） |
...
#### 31.2.4 ボーナスの期待ポイント率（U13・B案）

```
上乗せ率 = max(0, 特典額 − 差し引く年会費) ÷ 達成条件額
  特典額       = bonusValueYen（初年度の特典額・初回特典を含む。従来どおり）
  差し引く年会費 = カードの年会費。ただし初回特典（年会費永年無料）を達成済みなら0
```

加算する条件（狙う・未達成・期限内・対象の支払い方法）は従来どおり。マスタの値での上乗せ率：

| ボーナス | 特典 | 年会費 | 従来 | B案 |
|---|---|---|---|---|
| 三井住友カード ゴールド（NL） | 10,000＋初回5,500 | 5,500（初回達成後0） | 1.55% | 1.0% |
| PayPayカード ゴールド | 11,000（初年度6,000） | 11,000 | 1.1% | 0% |
| 三井住友カード プラチナプリファード | 10,000 | 33,000 | 1.0% | 0% |
| dカード GOLD | 10,000 | 11,000 | 1.0% | 0% |
| エポスゴールドカード | 10,000 | 5,000 | 1.0% | 0.5% |
| ビューカード スタンダード | 5,250 | 524 | 0.35% | 0.32% |
| マリオット・プレミアム | 75,000 | 82,500 | 1.875% | 0% |
| マリオット | 50,000 | 34,100 | 2.0% | 0.64% |

- 0% になるボーナスは「狙う」をオンにしても順位は変わらない。カード診断の見込み表示は従来どおり出す
- 影響：`engine.ts`（`bonusRate`）、参照実装・ゴールデンケース、理由の文言（「ボーナス1%相当（年会費差し引き後）」）
...
### 31.3 画面案（確認用）
`design/mockups/ui_review_2026-09-26.html`（スクリーンショット：同フォルダの PNG）。タブ名「カード診断」「ナビ」の両案、カードタブ、カード診断の［持っているカード］［おすすめカード］を示す。確認後にタブ名を決め、本章を確定して実装する。

### 31.4 テストの見込み
- 更新：E02（ボーナスの率）、E10〜E13・E19・E28・E29（見直すタブの文言・構成）、E14・E15（支払い方法の欄）、E16〜E18（タブ数）、E21（タグなし）、ゴールデン（1カード1枠・B案）
