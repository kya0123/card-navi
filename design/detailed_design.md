...
| 項目 | 内容 |
|---|---|
| 版数 | 1.20（カードラインナップの再整理：段階1〜4を実装） |
| 更新日 | 2026-09-26 |
| 前提 | 基本設計書 v1.13、ルールマスタ `master/rules.json`（masterVersion 2026.09.26-4） |
...
| 1.20 | 2026-09-26 | カードラインナップの再整理（32章。設計のみ・未実装）：一覧をシリーズ→ランクの2段構成に、シリーズごとに一般とゴールドを揃える、ポイントの評価（マイル1円・永久不滅ポイント5円・メンバーシップ・リワード0.3円）とANAワイドゴールドの率の変更、全カードで比べるときに同じシリーズ・同じ率のカードを1枠に、その他のカード（F14）、カード以外の支払い（d払い残高・楽天ペイ・au PAY・WAON・nanaco・楽天Edy）の追加と現金のみの店（32.11）、第2弾の収録カード（32.3・L12） |
...
## 32. カードラインナップの再整理（v1.20・2026-09-26。段階1〜4は実装済み）
...
| L12 | 第2弾 | 第2弾は、収録済みシリーズの上位カード（Olive プラチナプリファード、ソラチカカード、エポスプラチナ、三菱UFJカード プラチナ・アメックス、JCBプラチナ）とローソンPontaプラスだけを入れる。L1の「プラチナはポイント率に差があるものだけ」は、**基本の率か年間ボーナスのどちらかに差があれば入れる**と読む（32.3 第2弾） |
...
**第2弾**（利用者決定 2026-09-26。L12。率・特典は第1弾と同じく公開情報で調べてから登録する）

| シリーズ | 追加するカード | ランク | 入れる理由 |
|---|---|---|---|
| Olive | Olive フレキシブルペイ プラチナプリファード | プラチナ | 一般・ゴールドとポイント率が違う見込み |
| ANAカード | ANA To Me CARD PASMO JCB（ソラチカカード） | 一般 | ポイ活向けで人気。ANA JCB一般と還元ルールが違えば別のカード（L2）。同じなら ANA JCB一般の別名にする |
| エポスカード | エポスプラチナカード | プラチナ | 年間ボーナスがゴールドと違う（L12） |
| 三菱UFJカード | 三菱UFJカード プラチナ・アメリカン・エキスプレス・カード | プラチナ | 同上（率かボーナスの差を調査で確かめる） |
| JCBオリジナルシリーズ | JCBプラチナ | プラチナ | 同上 |
| ローソンPontaプラス（新シリーズ） | ローソンPontaプラス | 一般 | 定番。ローソン系の店舗はマスタにあるので、店舗の追加なしで比べられる。ゴールドは発行されていないので一般だけ（L1） |

- 調べた結果、率も年間ボーナスも同じシリーズの下位カードと変わらなければ入れない（楽天プレミアムと同じ扱い）
- ヨドバシ ゴールドポイントカード・プラス、ビックカメラSuicaカード、Orico Card THE POINT、ライフカードは入れない
- 第2弾まで入れると、登録は約47枚になる
...
- マスタ検証（`master.ts`・`build_master.py`）に次を加える
  ...
  - `tier` が既知の値である
  - シリーズ内に一般とゴールドの両方がある。ただし例外リスト（ゴールドが発行されていない Amazon Mastercard・リクルートカード・ローソンPontaプラス、一般のない Marriott）は除く
  - （実装）組の検査はマスタを作る `build_master.py` だけで行う。アプリの `validateMaster` はシリーズの存在・ランクの値・シリーズの名前とよみ・カードのないシリーズだけを検査する。段階3で追加するまで組が欠けているシリーズは `SERIES_PAIR_PENDING` に並べて検査を外し、組が揃ったのに残っていればエラーにする（外し忘れを防ぐ）
...
- 支払い方法は代表のカードのものを出す
- （実装）持っているカードは、ほかの枠にまとめない。同じシリーズ・同じ率のカードを2枚持っていれば、どちらも枠に出し、持っていないカードは上の持っているカードの枠に添える（持っているカードの優先順の情報を消さないため）
...
**実装**（`domain/custom.ts`）
- `withCustomCards(mi, customCards)` が、その他のカードをカード（`userDefined: true`、シリーズ `custom`）と経路（`custom_1_card_physical` など）としてマスタの索引に取り込む。`recommend` は最初にこれを呼ぶので、提案の計算・ゴールデンテストなど `recommend` を使う処理はそのまま動く。画面の `ctx.mi` も取り込んだ索引を返す
- 経路の `pointsPerUnit` は「率×100÷ポイントの円換算」（例：三菱UFJポイント（1pt＝5円）で1%なら0.2pt）。マスタ検証（`validateMaster`）の対象外
- 名前は30文字まで。IDは使い回さない（削除した後の追加は次の番号）
- 一覧（`cardCatalog`）・区分の枚数（`segmentCounts`）・「全N枚」・ポイント率の一覧（S05）は `masterCards(mi)` でその他のカードを除く
- ポイント払いの判定では、その他のカードのポイントも「持っているポイント」に入る
- 持っているカードの表示で外すと、登録も削除する。ポイント率の画面（S11）へのリンクは出さない（出典がないため）
...
| 3 | ポイントの評価（32.4）とANAの経路の変更、追加カードのマスタ登録（32.3）。ANA・楽天・三菱UFJ・au・JCB・イオン・Olive・ビュー・セブンを先に、JAL・セゾン・アメックスを続けて入れる。第2弾（32.3）はその後 | ルール版を更新 |
...
| 段階 | 状態 |
|---|---|
| 1 | 実装済み（2026-09-26）。ルール版 2026.09.26-2（シリーズ16件、カード22枚にシリーズとランクを付与。還元ルールは変更なし、ゴールデン48件は結果不変）。`compareCards(mi)` はシリーズ名 → ランク → カード名の順を返す比較関数に変更。一覧の見出しは `.catalog-series`、補足行の先頭に「ランク・発行会社」。単体：C07（シリーズ・ランクの検証）、C08（並び順）、C09（シリーズ名・別名・ランク名での検索）。E2E：E18 をシリーズ見出しの確認に変更 |
| 2 | 実装済み（2026-09-26）。その他のカード（`domain/custom.ts`、`reconcile`、S03 の登録欄・削除）と全カードで比べるときのまとめ（`engine.ts` の `mergeSameSeries`、S01 の「〜も同じポイント率」）。参照実装も同じ処理を追加。ゴールデン G49〜G52 を追加（既存48件は結果不変）。単体：L01〜L18（`tests/unit/lineup.test.ts`）、既存1件をまとめの仕様に合わせて変更。E2E：E32（その他のカード）・E33（まとめ）を追加、E18 を一覧部分の見出しに限定 |
| 3 | 実装済み（2026-09-26）。ルール版 2026.09.26-3（カード42枚・シリーズ20・ポイント18・経路178・特約581・ボーナス14）。調査結果と出典は `master/collection_report.md` 10章。第1弾17枚・第2弾3枚を追加。調査で変わった点：三菱UFJのゴールドは1種類（旧ゴールドプレステージの名称変更）、JCBプラチナと三菱UFJプラチナ・アメックスは下位カードとの差を確認できず入れない（L12）、ソラチカカードはANA JCB一般の別名。ANA VISAワイドゴールドは1.0%に（ゴールデン21件の期待値が変わった。いずれも2〜3位の並び）。提案のヒントの同率時の順を「年会費→ランク→カードID」に（分冊12.2）。ゴールデン G53〜G59、単体 C10 を追加 |
| 4 | 実装済み（2026-09-26）。ルール版 2026.09.26-4（支払い方法13・経路184・特約585）。調査結果と出典は `master/collection_report.md` 11章。楽天ペイは楽天キャッシュ払いの1.0%で評価（ポイントカード提示の条件付き1.5%は含めない）。使えるお店：コード決済はコンビニ・スーパー・ドラッグストアの既定、WAONはイオングループ・ミニストップ・ウエルシア（イオングループは2倍）、nanacoはセブン・イトーヨーカドー、楽天Edyはファミマ・ローソン・ナチュラルローソン。現金のみの店はマスタに該当なし（仕組みだけ実装）。コード：`defaultSettings`（`defaultEnabled`）、`Method.type` の `emoney`、`Route.label`、`Store.cashOnly`（マスタ検証・おすすめの注記・カテゴリ／近くのお店の「現金のみ」）、カードタブのチェックボックスの小見出し。参照実装は空の `acceptedMethods` を店の指定として扱うよう修正。ゴールデン G60〜G63（既存59件は結果不変）、単体 L20〜L23、E2E E34 を追加。現金のみの店はマスタに該当がないため、E2Eではなく単体（L23・テスト用マスタ）で確認。E2Eのコントラスト確認で無効なボタンを対象外にした（WCAG 1.4.3） |
