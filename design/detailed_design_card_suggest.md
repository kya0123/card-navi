...
| 項目 | 内容 |
|---|---|
| 版数 | 1.4（カードラインナップの再整理に伴う候補の拡大。設計のみ・未実装） |
| 作成日 | 2026-09-24（1.2：2026-09-25、1.3・1.4：2026-09-26） |
...
## 12. カードラインナップの再整理に伴う候補の拡大（v1.4・2026-09-26。設計のみ・未実装）

詳細設計32章でカードを約41枚に増やすことに伴う、提案（ヒント・バナー・カード診断の［おすすめカード］）への影響。

### 12.1 候補

- 候補は従来どおり「マスタにあり、持っていないカード」。約41枚のうち持っていないもの
- 「その他のカード」（詳細設計32.8）は候補にしない。利用者が持っているカードとして扱うので、持っているカードの年会費の判定（10.4）にも出さない（年会費を持たないため）

### 12.2 同じシリーズの一般とゴールド

- カード診断の［おすすめカード］は、年間の損得（還元の増加＋ボーナス−年会費）の大きい順に並べている（10.3）。同じシリーズで上がるお店が同じなら、年会費の分だけゴールドが下に来るので、並び順の規則は変えない
- ヒント（①）は「3ポイント以上上がるカード」を1枚出す。同じ率のカードが複数あるときは、年会費の安いカードを出す（`bestUnownedAt` の同率の順を「年会費の安い順 → カードID」に変える。従来はカードIDの順）
- 全カードで比べるときのまとめ（詳細設計32.7）は、おすすめタブだけの仕様で、提案には使わない

### 12.3 テスト

| ID | 観点 |
|---|---|
| S06 | `bestUnownedAt` の同率は、年会費の安い順 → カードID順に変える |
| S08・S09 | 候補が増えたときの動きの期待値を見直す |
| 追加 | その他のカードが候補・年会費の判定に出ないこと。ヒントで同じ率なら年会費の安いカードが出ること |
