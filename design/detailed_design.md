...
| 1.20 | 2026-09-26 | カードラインナップの再整理（32章。設計のみ・未実装）：一覧をシリーズ→ランクの2段構成に、シリーズごとに一般とゴールドを揃える、ポイントの評価（マイル1円・永久不滅ポイント5円・メンバーシップ・リワード0.3円）とANAワイドゴールドの率の変更、全カードで比べるときに同じシリーズ・同じ率のカードを1枠に、その他のカード（F14） |
...
L8について：Marriott（100円＝3pt／2pt）と楽天プレミアム（1.0%）は、どちらも使った金額でポイント率が変わらない。使った金額で変わるのは、Marriottの年間利用額に応じた無料宿泊特典だけで、これはボーナスとしてマスタに入れ済み（21章）。
...
| 楽天カード | `rakuten` | 新 楽天ゴールドカード | 新 楽天プレミアムカード |
| リクルートカード | `recruit` | （発行なし） | — |

- 新しく加えるのは約20枚（JALの発行会社別の枚数は、L2に沿って還元ルールを調べてから決める）。登録は約42枚になる
- 楽天プレミアムカードの基本は楽天カードと同じ1.0%で、L1の「プラチナはポイント率に差があるものだけ」には当たらない。利用者が第1弾の候補に挙げたため残す。マスタに入れる前に、外すかどうかを利用者に確認する
- 楽天ゴールド・楽天プレミアム・JCBゴールドのように、このアプリ上では「ポイント率は一般カードと同じか低く、年会費だけ高い」カードもある。一覧を揃えるために収録する
...
**課題**：持っていないカードにはボーナスを加算しない（30.1 R5）ため、同じシリーズの一般とゴールドは、ほとんどの店で同じ率になる。全カードで比べると、同じシリーズのカードが上位3枠を占め（例：楽天市場で楽天カード・楽天ゴールド・楽天プレミアム）、他のシリーズが見えなくなる。
...
- まとめたほかのカードは `RecommendItem.sameRateCardIds?: Id[]` に入れる。画面では代表のカード名の下に「楽天ゴールドカード・楽天プレミアムカードも同じポイント率」と添える
- 支払い方法は代表のカードのものを出す
